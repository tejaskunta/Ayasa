...
function SplashCursor({
  ...
  BACK_COLOR = { r: 0.039, g: 0.055, b: 0.102 },
  TRANSPARENT = true
}) {
  ...
  useEffect(() => {
    ...
    // Theme-aware color generation — biased toward teal (#44E5C2) and lavender (#A78BFA)
    // Hue buckets: teal ~0.46, lavender ~0.73, with occasional soft cyan/blue
    const THEME_HUES = [0.46, 0.48, 0.50, 0.73, 0.75, 0.60, 0.65];
    function generateColor() {
      const h = THEME_HUES[Math.floor(Math.random() * THEME_HUES.length)] + (Math.random() - 0.5) * 0.04;
    ...
    function handleMouseMove(e) {
      ...
      if (!firstMove) { updatePointerMoveData(pointer, posX, posY, generateColor()); firstMove = true; }
